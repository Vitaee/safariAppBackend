                ...
                async with session.get(f"{self.base_url_home}{counter}") as response:
                    ...
                    for tour in tours:
                        ...
                        if data_over_view != 'Tour is unavailable':
                            data_day_by_day = await self.scrape_day_by_day_details(session, url.split('t')[-1])
                            data_inclusions = await self.scrape_inclusions_details(session, url.split('t')[-1])
                            data_getting_there = await self.scrape_getting_there_details(session, url.split('t')[-1])
