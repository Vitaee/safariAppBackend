...
class TestUserView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.safari_url = '/api/safari/create'
        data = {
        ...
        response = cls.client.post(cls.safari_url, data, format='json')
        cls.assertEqual(response.status_code, status.HTTP_201_CREATED)
        cls.safari_id = response.data['id']

    def test_post_user_favs(self):
        url = reverse('accounts:user_favs')
        data = {
            "user": self.user.id,
            "safari": self.safari_id
